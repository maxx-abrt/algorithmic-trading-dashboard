...
const REGIME_TILT: Record<Regime, Partial<Record<Factor['group'], number>>> = {
  TRENDING_UP: { trend: 1.45, momentum: 1.2, mtf: 1.3, structure: 0.9, volatility: 0.55 },
  TRENDING_DOWN: { trend: 1.45, momentum: 1.2, mtf: 1.3, structure: 0.9, volatility: 0.55 },
  RANGING: { trend: 0.6, structure: 1.5, volatility: 1.1, momentum: 0.95, pattern: 1.15 },
  ...
  CHOPPY: { trend: 0.45, momentum: 0.6, structure: 1.2, pattern: 0.6, mtf: 0.8 },
}
...
export function buildVetoes(input: {
  ...
  const { indicators: i, mtf, derivatives, settings, side, alignment, dataWarnings } = input
  const v: Veto[] = []
  ...
  // Higher timeframes must not point the other way for trend playbooks.
  const htf = mtf.filter((t) => t.timeframe !== settings.timeframe)
  const opposed = htf.filter((t) => (long ? t.trendScore < -25 : t.trendScore > 25))
  if (opposed.length) {
    v.push({
      id: 'htf_conflict',
      reason: `${opposed.map((t) => `${t.timeframe} ${t.bias}`).join(', ')} opposes a ${side}.`,
      severity: settings.requireMtfAlignment ? 'hard' : 'soft',
    })
  }
  ...
  // No room = no trade, whatever the indicators say.
  ...
  if (barrier) {
    ...
    if (room < 0.8 && barrier.strength > 55) {
      v.push({
        id: 'no_room',
        reason: `Strong ${barrier.kind} at ${barrier.price.toPrecision(6)} only ${room.toFixed(2)} ATR away (strength ${barrier.strength.toFixed(0)}).`,
        severity: 'hard',
      })
    }
  }
  ...
  for (const w of dataWarnings) {
    v.push({ id: 'data', reason: w, severity: w.includes('stale') ? 'hard' : 'soft' })
  }
...
export function decide(input: {
  ...
  let conviction =
    Math.abs(comp) * 0.5 + agreement * 0.28 + alignment * 0.22 - softVetoes.length * 6
